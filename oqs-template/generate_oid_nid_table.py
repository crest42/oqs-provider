...
def gen_sig_table(oqslibdocdir, config):
        ...
        for variant in sig["variants"]:
            ...
            try:
                ...
                for hybrid in variant["mix_with"]:
                    table.append(
                        [
                            variant["name"] + " **hybrid with** " + hybrid["name"],
                            liboqs_sigs[sig["family"]]["spec-version"],
                ...
                for i in range(len(variant["extra_nids"]["old"])):
                    table.append(
                        [
                            ...
                            claimed_nist_level,
                            variant["extra_nids"]["old"][i]["code_point"],
    ...
    with open(
        os.path.join("oqs-template", "oqs-sig-info.md"), mode="w", encoding="utf-8"
    ) as f:
        f.write(tabulate(table, tablefmt="pipe", headers="firstrow"))
    print("Written oqs-sig-info.md")


def gen_kem_table(oqslibdocdir, config):
    ...
    for kem in sorted(config["kems"], key=lambda k: k["family"]):
        ...
        if "implementation_version" in kem:
            ...
            if kem["family"] in liboqs_kems:
                ...
                raise KeyError("No implementation version found for " + kem["family"])
        ...
        try:
            table.append(
                [
                    ...
                    implementation_version,
                    kem["name_group"],
                    str(liboqs_kems[kem["family"]]["nist-round"]),
                    claimed_nist_level,
                    kem["nid"],
                    "",
                ]
        ...
        if "extra_nids" in kem:
            if (
                "current" in kem["extra_nids"]
            ):  # assume "current" NIDs to mean liboqs-driven NIST round information:
    ...
    table.sort(
        key=lambda row: "{:s}|{:s}|{:s}|{:s}|{:s}".format(
    ...
    with open(
        os.path.join("oqs-template", "oqs-kem-info.md"), mode="w", encoding="utf-8"
    ) as f:
        f.write(tabulate(table, tablefmt="pipe", headers="firstrow"))
        f.write("\n")
...
def generate_oid_nid_table_main():
    with open(
        os.path.join("oqs-template", "generate.yml"), mode="r", encoding="utf-8"
    ) as f:
    ...
    gen_kem_table(oqsdocsdir, config)
    gen_sig_table(oqsdocsdir, config)
